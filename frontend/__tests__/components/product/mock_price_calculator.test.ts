...
import { PriceCalculator, TaxProps } from '@/app/components/product/price_calculator'
...
describe('Test Mock PriceCalculator', () => {
  ...
  it('check roundDecimal', () => {
    const got = mockCurrency.roundDecimal(123.456, 2)
    const want = 123.46
    expect(got).toStrictEqual(want)
  })

  it('check roundDigit', () => {
    const got = mockCurrency.roundDigit(123456, 3)
    const want = 123000
    expect(got).toStrictEqual(want)
  })
  ...
  it('check return productType', () => {
    const prodctType = mockCurrency.productType(productUS)
  ...
  it('calc ProductPrice', () => {
    const got = mockCurrency.calcProductPrice(productUK, storeUk)
    console.log(got)
    const want = {
      currencyCode: 'GBP',
      retailPrice: 39.99,
      ...
      taxReducedSalePrice: 28.33,
      RetailKRWPrice: 56900,
      KRWPrice: 48300,
      saleRate: 0.15,
    }
    expect(got).toStrictEqual(want)
  ...
  it('calc Tax', () => {
    ...
    const productType = mockCurrency.productType(productUS)
