...
import {
  Dialog, DialogTrigger, DialogContent, DialogClose, DialogFooter,
...
function loadMobileWarningState() {
  ...
  const mobileWarningValue: number | null = mobileWarningState
              && JSON.parse((mobileWarningState));
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[90vw] max-w-[400px] max-h-[500px] flex-center gap-1 rounded-lg">
        ...
        <div>
          <span className="font-bold underline">PC/태블릿 환경</span>
          을 이용해주세요.
        </div>
        <DialogFooter className="sm:justify-start pt-4">
          <DialogClose asChild>
            <Button type="button" variant="secondary" asChild={false} onClick={closeHandler}>
              닫기
            </Button>
          </DialogClose>
        </DialogFooter>
