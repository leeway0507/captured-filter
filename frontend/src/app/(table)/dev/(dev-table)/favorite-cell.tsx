...
import { ProductTableProps } from '../../product/price-calculator';

function getFavoriteIdList(): Array<number> {
  const s = localStorage.getItem('dev_av_i'); // favoriteId
...
function removeFavoriteIdList(rowId:number) {
  ...
  localStorage.setItem('dev_av_i', encodedString);
}
...
function addFavorite<H extends ProductTableProps>(props: CellContext<H, any>) {
  ...
  if (favoriteList) {
    ...
    if (!isExisted) {
      ...
      // dev_f_rd = favorite
      localStorage.setItem('dev_f_rd', encodedString);
