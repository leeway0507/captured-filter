...
import FilterButton from '@/app/(table)/components/button';
import { SaveFavoritePersonalOption, FavoriteOptionsProps } from '../dev-favorite/favorite-table/options';
import { useDevFavorite } from './context';

export default function Filter() {
  const { getFavoriteOptions, setFavoriteOptions } = useDevFavorite();
  ...
  const FilterContent = (
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        ...
        <div className="grid grid-cols-5 items-center gap-4">
          <Label className="col-span-3" htmlFor="width">물품수(개)</Label>
          <Input
            id="numOfProduct"
            defaultValue={changedOptions?.numOfProduct}
            className="col-span-2 h-8"
            type="number"
            min="0"
            onChange={(e) => {
              const newOptions: FavoriteOptionsProps = {
                ...changedOptions!, // assuming `changedOptions` is your current state
                numOfProduct: Number(e.target.value),
              };
              setOptions(newOptions);
            }}
          />
