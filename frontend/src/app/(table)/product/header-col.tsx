...
import {
  Table, ColumnFiltersState, Column,
...
function BrandOptionComponent(props: OptionProps<BrandValueProps>) {
  const { data } = props;
...
function StoreOptionComponent(props: OptionProps<StoreValueProps>) {
  const { data } = props;
...
export function AccordianFilter({ columnArr, columnName, setSelectedValue }:
...
{ table:Table<ProductTableProps>, columnName:string }) {
  ...
  const buttonComponent = (
    <div className="flex justify-end w-full gap-2 p-2">
      ...
      <DialogClose asChild>
        <Button asChild={false} className="px-6" onClick={onClickHandler}>확인</Button>
      </DialogClose>
    </div>
  );
