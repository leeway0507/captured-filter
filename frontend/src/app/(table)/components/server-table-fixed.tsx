...
import { ConvertFilterToQueryString, ConvertPageToQueryString } from '@/app/(table)/components/server-filter';
import cn from '@/lib/utils';
import { LeftArrowButton, RightArrowButton } from '@/app/(table)/components/button';
...
function ServerTableFixed<TData, TValue>({
  ...
  const scroll = (scrollOffset: number) => {
    elementRef!.current?.scrollTo({ left: scrollOffset, behavior: 'smooth' });
  ...
  return (
    <div className="w-full max-w-[1024px] mx-auto scroll-bar-y-hidden">
      <LeftArrowButton
        className={cn('max-lg:hidden absolute left-[calc(50vw-550px)] top-[calc(50vh-10px)]', scrollX === 0 && 'hidden')}
        onClick={() => scroll(0)}
      />
      <RightArrowButton
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    const cellSticky = columnPin.findIndex((o) => o.id === cell.column.id);
                    return (
                      <TableCell
                        key={cell.id}
